...
namespace YoutubeTranscriptApi
{
    ...
    internal sealed class TranscriptListFetcher
    {
        ...
        {
            var (title, captions) = extractCaptionsJson(fetchVideoHtml(videoId), videoId);
        ...
        internal static (string, JsonElement) extractCaptionsJson(string html, string videoId)
        {
            ...
            var titlePrep = splitted_html[1].Split(",\"videoDetails\":")[1];
            titlePrep = titlePrep.Split("\"title\":\"")[1];
            var title = titlePrep.Split("\",\"")[0];

            //var title = JsonSerializer.Deserialize<JsonElement>(splitted_html[1].Split(",\"videoDetails\":")[1].Replace("\n", "")).GetProperty("title").GetString();

            return (title, captions_json);
        }
    ...
    public class TranscriptList : IEnumerable<Transcript>
    {
        ...
        internal TranscriptList(string videoId, string videoTitle, Dictionary<string, Transcript> manuallyCreatedTranscripts, Dictionary<string, Transcript> generatedTranscripts, List<Dictionary<string, string>> translationLanguages)
        {
            VideoId = videoId;
        ...
        public static TranscriptList build(HttpClient httpClient, string videoId, string videoTitle, JsonElement captionsJson)
        {
